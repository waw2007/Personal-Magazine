...
| 文件 | 内容 | 是否提交 git |
| --- | --- | --- |
| `news.json` | 爬虫原始输出（约 79 条） | ❌ |
| `important_news.json` | 筛选后的重要信息（约 12 条） | ❌ |
...
2. **网页变更检测** — 避免重复推送同一条通知（比较 URL 或标题哈希）
3. **抓取频率配置化** — 目前写死每天 08:00，可改为 per-site 频率
4. **前端体验增强** — 已读标记、信息归档、移动端适配

### 中期（需要设计）
5. **聊天群消息监控** — 监控微信/QQ 群的关键词，⚠️ 涉及隐私与平台合规，需先评估法律风险
6. **多 Agent 协作** — 抓取、理解、决策、提醒等多个 Agent 分工
7. **信息智能评分模型** — 从规则打分升级为可学习的评分

### 远期
8. **上云部署** — 公网访问 + 24 小时在线（当前是本地常驻，关机即停）
9. **多用户 / 多学校** — 从个人工具走向可配置的通用产品
